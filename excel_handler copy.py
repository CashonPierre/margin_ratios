...
# Helper class to handle the asynchronous IBKR callbacks
class IBMarginClient(EWrapper, EClient):
    def __init__(self):
        EClient.__init__(self, self)
        self.results_queue = queue.Queue()

    def error(self, reqId, errorCode, errorString, advancedOrderRejectJson=""):
        # Log real errors (exclude status notifications like 2104)
        if errorCode > 2000: return 
        if errorCode > 0:
            print(f"IBKR Error {errorCode}: {errorString}")
            if errorCode == 200: # No security definition found
                self.results_queue.put({"Error": f"Symbol not found: {errorString}"})

    def openOrder(self, orderId, contract, order, orderState):
        """Captured the What-If results"""
        result = {
            'Symbol': contract.symbol,
            'initMarginChange': float(orderState.initMarginChange),
            'maintMarginChange': float(orderState.maintMarginChange),
            'equityWithLoanAfter': float(orderState.equityWithLoanAfter),
            'minCommission': orderState.minCommission,
            'maxCommission': orderState.maxCommission,
            'currency': orderState.commissionCurrency,
            'Error': None
        }
...
# to be implemented
def process_for_sheet4(item):
    """
    Calls IBKR TWS API for What-If margin impact.
    """
    symbol = item.strip().upper()
    print(f"IBKR: Checking {symbol}...")
    
    app = IBMarginClient()
    # Connect to TWS (7497 for Paper, 7496 for Live)
    ...
    # Define "What-If" Order
    order = Order()
    order.action = "BUY"
    order.totalQuantity = 100 # Adjust quantity as needed for margin comparison
    order.orderType = "MKT"
    order.whatIf = True
    order.eTradeOnly = False
    order.firmQuoteOnly = False

    # Place order and wait for the queue
    try:
        # Give a small buffer for connection
        time.sleep(1)
        # We use orderId 1 for each fresh connection
        app.placeOrder(1, contract, order)
        
        # Wait up to 5 seconds for the result
        result = app.results_queue.get(timeout=5)
    except queue.Empty:
        result = {'Symbol': symbol, 'Error': 'Timeout - No data from TWS'}
    except Exception as e:
...
for item in item_list:
    try:
        ...
        res4 = process_for_sheet4(item)
        results_sheet4.append(res4)
        ...
        results_sheet4.append({'Error': str(e)})
...
with pd.ExcelWriter(OUTPUT_FILE, engine='openpyxl') as writer:
    ...
    df_sheet4.to_excel(writer, sheet_name=SHEET4_NAME, index=False)

print("Done! Check the output file for Sheet2, Sheet3, and Sheet4.")